	...
	get_jsapi_ticket()
		.then(function(ticket) {
			...
			var data = {
				...
				signature: "",
				jsApiList: ["chooseImage", "uploadImage", "chooseWXPay", "previewImage"]
			}
					...
					.then(function(msg) {
						if (msg && msg.ticket) {
							if (global.config.cache.enable) {
								cache.set("jsapi_ticket", msg.ticket, msg.expires_in - 100);
							}
	...
	request.send(url, {})
		.then(function(msg) {
			if (msg && msg.access_token) {
				if (global.config.cache.enable) {
					cache.set("access_token", msg.access_token, msg.expires_in - 100);
				}
